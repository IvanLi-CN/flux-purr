...
- `FUSB302BMPX` is a PD sink PHY at 7-bit `0x22`, with its `GPIO7` interrupt net reserved for a later event-driven path. The polling policy applies the `5V..28V` PD guard before selecting its live PPS APDO (currently `5V..21V`) and retains fixed-PDO fallback; source interoperability remains an HIL gate.
