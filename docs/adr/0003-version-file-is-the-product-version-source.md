...
- Every CI-verified non-Release Commit on `main` is a separate product release. The release controller must create its Release Commit before another product-source commit may merge; it must never coalesce multiple source commits into one version. The normal version transition is `nextPatch(VERSION)` and has no input other than `VERSION`; PR labels, Git tags, package manifests, snapshots, and workflow channels cannot change it. A major, minor, or prerelease release is an explicit Release Commit that writes its exact SemVer text to `VERSION`; after that commit, no downstream step reads a separate version input.
