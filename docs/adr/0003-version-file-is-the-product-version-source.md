...
- Every CI-verified non-Release Commit on `main` with enabled release intent is a separate product release. The release controller must create its Release Commit before another product-source commit may merge; it must never coalesce multiple source commits into one version. The normal `type:patch` transition is `nextPatch(VERSION)`. `type:minor`, `type:major`, and `channel:rc` are validated PR intent selectors: they choose the controlled bump operation, while the numeric value is still computed from the checked-out `VERSION` and written once to the Release Commit. Git tags, package manifests, snapshots, and workflow channels cannot override that file. After the Release Commit, no downstream step reads a separate version input.
- The release controller may create a Release Commit only after its source commit has passed CI and only when its diff is limited to `VERSION`. It tags, publishes, and verifies the release before fast-forwarding `main` to that commit. `main` therefore needs an enforced release-completion gate before the next product-source merge. The release artifact jobs run once per source commit without repeating the full CI matrix, and must ignore the Release Commit push to avoid recursive CI or release runs.
