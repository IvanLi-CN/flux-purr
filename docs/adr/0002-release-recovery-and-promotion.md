...
Promotion state is stored separately in `refs/notes/release-promotions`. A schema-v1 record contains the candidate SHA, canonical digest of the source snapshot, candidate tag, source channel, stable channel, effective version, and stable tag. A repeated operation must match the existing record byte-for-byte. A stable tag may be reused only when the matching promotion record exists, the tag points at the candidate, and any existing Release manifest agrees on source, version, channel, component identities, and asset hashes; a conflicting tag or manifest fails.
