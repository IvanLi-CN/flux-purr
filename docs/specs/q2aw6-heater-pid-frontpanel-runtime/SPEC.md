...
- Given 旗舰目标温度集调优正在执行，When target 属于 `60 / 140 / 220°C`，Then 单个目标必须在 `20min` 预算内执行 tuning scout、retune、最多两轮定向 batch candidate compare、最多一次 `60s` Hold confirm；confirm target-local 失败即结束该 target，不得派生恢复 scout 或再次 confirm。默认流程不得额外采集 `0% / 25% / 50%` approach-only 曲线；历史曲线只可作为显式诊断资料引用，不得进入当前默认跑法。
