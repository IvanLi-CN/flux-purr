...
- 两个 bank 各 10 个完整 point-local 目标点必须能与最长 Wi-Fi 凭据和完整校准状态同时持久化；EEPROM 使用 `2 KiB` v3 active 双槽和 `u16` TLV 长度，读取兼容 `1 KiB` v2 与 `512B` legacy 槽；EEPROM 不可达时 flash fallback 使用同一 record 编码和 sequence 选择规则。
