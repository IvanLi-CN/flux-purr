...
- Given 旗舰目标温度集调优正在执行，When target 属于 `60 / 140 / 220°C`，Then 单个目标必须在 `20min` 预算内执行 tuning scout、retune、最多三轮定向 batch candidate compare、最后一次 `60s` Hold confirm；confirm target-local 失败且预算仍充足时，允许一次“定向修正点 short scout + confirm”恢复链。默认流程不得额外采集 `0% / 25% / 50%` approach-only 曲线；历史曲线只可作为显式诊断资料引用，不得进入当前默认跑法。
...
- Given preliminary flagship review bundle 在 `100w / pps5a` 下执行，When 目标集合是 `60 / 140 / 220°C`，Then bundle 必须包含 `index.html`、`run.bundle.json`、`samples.ndjson` 与 `thermal-profile.accepted.json`，并且 `run.bundle.json` 顶层与页面事实区都必须回显 `selectedMode=100w`、`resolvedBank=pps5a`、`detectedSourceClass=pps5a`、`tuningBudgetSeconds` 与 `flagshipTargetsC`。每个目标温度 tab 必须按实际执行顺序展示所有具备有效样本的 scout、batch candidate、confirm 与 confirm-recovery attempt，明确候选名、是否被采用、证据是否有效、失败分类、参数与评价，并提供对应温度响应、控制输出和 source telemetry 图。环境故障 attempt 可保留审计摘要，但不得混入 `validTestCount` 或候选评分。该 preliminary bundle 不代表 EEPROM saved bank，也不代表 committed accepted baseline。
