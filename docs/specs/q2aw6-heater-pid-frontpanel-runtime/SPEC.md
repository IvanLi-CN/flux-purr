...
- 提供 CLI/devd 自测试入口，抽象 bench source provider；当前默认且验收支持的 provider 是 IsolaPurr released CLI，用于准备 `65W`、PD Fixed enabled、PPS enabled、`auto_follow` 外部 source，输出 `run.json`、`samples.ndjson`、`thermal-profile.candidate.json` 与带图表的 `report.html`。
...
- 每个有效 RTD 批次的原始温度必须先独立执行 overtemp 判定；控制器、前面板与 runtime status 使用最近 3 个有效批次的中位温度，启动时 1 个样本直通、2 个样本取均值。该跨 tick 稳健估计不得延迟 short/open/overtemp 故障，任一传感器 fault 必须立即清空温度缓存并保持既有加热关断路径。PPS request 发生变化时，控制温度保持最近可信值，直到 request 连续 `300ms` 未变化；恢复时必须同时重新播种 RTD temporal window 与 controller filtered temperature/slope，避免把电压切换耦合或播种跳变量化为虚假升温率。原始 short/open/ADC-read/overtemp 检查在该窗口内仍须逐周期执行。
