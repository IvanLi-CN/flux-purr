...
- HTML summary card 与轮次详情 MUST 显示两个独立时间：`逼近用时 = guard.firstHoldAtMs - guard.approachStartedAtMs`（兼容旧 bundle 时可用 `fullSpeedToStable.warmupExitedAtMs` 作为起点），以及 `稳定用时 = result.fullSpeedToStable.settleTimeMs`；两者都必须直接来自同一轮次的时间戳，不得把不连续的多个 `approach` 相位样本首尾相减后作为主展示耗时。稳定窗口门槛 `limitMs` 必须单独显示并与稳定用时比较，不能把门槛本身当作加热用时。
...
- 3A 九点 thermal-plant 合规摘要：9 个目标均为 `passed`，摘要卡直接显示逼近用时、稳定用时、动态门槛、过冲与 hold 峰峰值；报告源为 `pps3a` / `f293cc9c139e`。
