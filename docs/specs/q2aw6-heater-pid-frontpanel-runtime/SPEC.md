...
- thermal batch 的调优锚点固定为 `60 / 100 / 140 / 180 / 220°C`，验证目标固定为 `80 / 120 / 160 / 240°C`，按低温到高温顺序执行；`250°C` 不参与参数调优。旗舰目标温度集固定为 `60 / 140 / 220°C`，其余目标温度默认只在旗舰三点通过后再按相邻锚点插值扩展。每轮开始前必须主动降温；重启阈值固定为：`targetTempC < 80°C => currentTempC <= 35°C`，`targetTempC >= 80°C => currentTempC <= targetTempC - 40°C`。温度首次达到或低于阈值后必须立即开始该轮，不得要求回到 `30°C`、不得增加固定 `30s` 或其它稳定等待，也不得跳过冷却。
...
- Given thermal batch 将运行目标 `T`，When 上一轮结束并主动降温，Then 若 `T < 80°C`，host 必须等待到 `currentTempC <= 35°C` 后立即开始；若 `T >= 80°C`，则必须等待到 `currentTempC <= T - 40°C` 后立即开始；不得附加低于 `30°C`、连续稳定时长或无冷却启动条件。旗舰目标温度集固定为 `60 / 140 / 220°C`，调优先只收这三点；`80 / 120 / 160 / 240°C` 只用于相邻锚点插值验证，`250°C` 不参与调优。
...
- Given 旗舰目标温度集调优正在执行，When target 属于 `60 / 140 / 220°C`，Then 单个目标必须在 `20min` 预算内执行 tuning scout、target-local retune、`current + 一个 evidence-specific predicted point` 的 batch compare，并在单目标预算仍有剩余时持续重复同一 target-local scout/retune/batch/confirm 闭环；每当出现满足 warmup `100%`、动态 full-speed-to-stable gate 与确认裕量的 promotable candidate 时，都必须进入一次 `60s` Hold confirm。若 Hold confirm 以 target-local 热控失败结束但预算仍在，Then 必须保留该次证据、reseed 下一轮候选，并继续同一 target，直到 `completed`、真实 `budget_exhausted` 或 `environment_blocked`。默认流程不得额外采集 `0% / 25% / 50%` approach-only 曲线；历史曲线只可作为显式诊断资料引用，不得进入当前默认跑法。
...
- Given preliminary flagship review bundle 在 `100w / pps5a` 下执行，When 目标集合是 `60 / 140 / 220°C`，Then bundle 必须包含 `index.html`、`run.bundle.json`、`samples.ndjson` 与 `thermal-profile.accepted.json`，并且 `run.bundle.json` 顶层与页面事实区都必须回显 `selectedMode=100w`、`resolvedBank=pps5a`、`detectedSourceClass=pps5a`、`tuningBudgetSeconds` 与 `flagshipTargetsC`。每个目标温度 tab 必须按实际执行顺序展示所有具备有效样本的 scout、batch candidate 与最多一次 confirm，明确候选名、是否被采用、证据是否有效、失败分类、参数与评价，并提供对应温度响应、控制输出和 source telemetry 图。环境故障或被中止的非计划 attempt 可保留排除审计摘要，但不得混入 `validTestCount` 或候选评分。该 preliminary bundle 不代表 EEPROM saved bank，也不代表 committed accepted baseline。
