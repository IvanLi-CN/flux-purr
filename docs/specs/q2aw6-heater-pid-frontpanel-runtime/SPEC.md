...
- 当 approach 的有效 PPS floor 为 CH224Q `5000mV` 时，`GPIO47` 必须保持每个 `50ms` tick 离散 `0% / 100%`，并按控制请求功率相对当前活动 PPS 电压的静态功率作确定性的 pulse-density 分配；该补偿必须覆盖 bounded PPS down-ramp 与 `5V` floor，避免控制功率已下降但活动电压仍较高时持续全开而把残余热带入 hold。hold 仍使用其既有回补滞回；不得启用硬件 PWM 或改变高于 `5V` floor 的 profile 行为。
...
- Given 5A full-batch 调优正在执行，When tuning target 属于 `60 / 100 / 140 / 180 / 220°C`，Then 单个目标必须在 `20min` 预算内执行 tuning scout、target-local retune、`current + 一个 evidence-specific predicted point` 的 batch compare，并在单目标预算仍有剩余时持续重复同一 target-local scout/retune/batch/confirm 闭环；每当出现满足 warmup `100%`、动态 full-speed-to-stable gate 与确认裕量的 promotable candidate 时，都必须进入一次 `60s` Hold confirm。若 Hold confirm 以 target-local 热控失败结束但预算仍在，Then 必须保留该次证据、reseed 下一轮候选，并继续同一 target，直到 `completed`、`not_converged`、`candidate_ready`、真实 `budget_exhausted` 或 `environment_blocked`。When 所有 tuning targets 收口后，Then validation targets `80 / 120 / 160 / 240°C` 必须用最终 review candidate profile 执行 `60s` hold-confirm validation；validation 通过只记录 `validation_passed`，不触发调优、不新增 profile 锚点；validation 未通过只记录失败证据，不得静默修改 profile。默认流程不得额外采集 `0% / 25% / 50%` approach-only 曲线；历史曲线只可作为显式诊断资料引用，不得进入当前默认跑法。
