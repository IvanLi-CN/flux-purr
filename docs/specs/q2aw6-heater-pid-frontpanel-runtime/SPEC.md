...
- 若不把风扇策略、过温停热、12V PD 默认请求与前面板显示一次收口，后续板级调试会持续混淆“策略开关”“实际输出”“保护联动”三套状态。
...
- 冻结正式风扇/保护包线：
  - active cooling `ON`：`<35°C` 停止、`>40°C` 最低电压启动、`>50°C` 全速，`35~40°C` 保持回差。
...
- 默认启动时把 CH224Q 请求从 `20V` 改为 `12V`，并保持外部 `fan_enabled + fan_pwm_permille` 归一化契约不变。
...
- CH224Q 在启动时默认请求 `12V`；PD 状态变化只允许进入日志/状态观测，不得触发 heater 锁死或自动关断。
...
- `Active Cooling` 页面在正式 runtime 中为只读安全策略说明页，必须同步 `12V`、`<35 / >40 / >50 / >100 / >350 / >360°C` 新包线。
...
- 启动后先请求 `12V`，随后初始化 RTD、heater PWM、fan 运行态和前面板 UI。
...
- Given active cooling 开启，When 温度 `<35°C / >40°C / >50°C`，Then fan 必须分别进入停止 / 最低电压 / 全速，并在 `35~40°C` 维持回差。
...
- 继续把 CH224Q 作为电源准备层而不是 heater interlock，只把 `12V` 请求和观测日志纳入当前合同。
