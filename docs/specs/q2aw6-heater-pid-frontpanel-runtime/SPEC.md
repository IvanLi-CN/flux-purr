...
- HTML summary card 与轮次详情中的 full-speed 实测值 MUST 直接读取 `result.fullSpeedToStable.settleTimeMs`，并与同一对象的 `limitMs` 比较；不得把不连续的多个 `approach` 相位样本首尾相减后作为门禁或主展示耗时。
