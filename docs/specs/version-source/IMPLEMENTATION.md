...
- 当前 active GitHub ruleset 没有允许 release workflow identity 的 bypass，不能让现有 workflow token 创建受保护 `main` Release Commit。仓库还同时保留 classic branch protection；两者会同时生效。owner 必须先将 classic protection 收敛到 ruleset，并为现有 release workflow identity 配置最小必要的 bypass；发布流程不依赖额外 GitHub environment。该远端配置由 owner 执行，不在仓库代码变更范围内。
...
2. owner 将现有 classic branch protection 的保护迁移到 `main` ruleset 并移除 classic rule，为现有 release workflow identity 配置最小必要的 `contents: write` bypass。同步 ruleset 与 `.github/quality-gates.json`：保留 `Validate PR labels`，并同时要求 `Release completion`、Firmware、DEVD、Web 和 Worktree checks。
