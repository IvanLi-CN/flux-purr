...
- 当前 active GitHub ruleset 没有 release App bypass，不能让 release controller 创建受保护 `main` Release Commit。仓库还同时保留 classic branch protection；两者会同时生效。owner 必须先将 classic protection 收敛到 ruleset，安装只具备 `contents: write` 的 release App，配置 repository variable `RELEASE_APP_ID` 和 secret `RELEASE_APP_PRIVATE_KEY`，并将该 App 设为唯一 always-bypass actor。发布流程不依赖 GitHub Environment；该远端配置由 owner 执行，不在仓库代码变更范围内。
...
2. owner 将现有 classic branch protection 的保护迁移到 `main` ruleset 并移除 classic rule，安装 release App，配置 `RELEASE_APP_ID` / `RELEASE_APP_PRIVATE_KEY`，并只为该 App 配置最小必要的 `contents: write` always-bypass。同步 ruleset 与 `.github/quality-gates.json`：保留 `Validate PR labels`，并同时要求 `Release completion`、Firmware、DEVD、Web 和 Worktree checks。
