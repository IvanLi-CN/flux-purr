...
- 启动屏的可见内容范围必须落在逻辑行 `y=4..45`，使 `160x50` 画布的顶部和底部各保留 `4` 个逻辑像素的留白。
- splash 的最终帧必须严格使用四种颜色：背景 `#08111F`、机身/项目名 `#F7FBFF`、热区 `#FF5542`、版本与抗锯齿细节 `#8999AD`。长度不超过 `18` 字符的版本必须使用专用 `4×7` 像素字形叠加到模板中；仅超长版本可降级为 `3×5` 字形，确保开发构建仍显示带 short SHA 的 build identity。
