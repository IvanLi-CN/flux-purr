...
- `active`: the display driver remains current and physical preview behavior is maintained by the RAM bring-up topic.
...
- The branded splash uses a checked-in RGB565 template derived from the official Logo asset, rather than runtime approximations of the mark or a general-purpose font. Wordmark edges preserve per-pixel SVG alpha coverage through theme-aware RGB565 blending.
