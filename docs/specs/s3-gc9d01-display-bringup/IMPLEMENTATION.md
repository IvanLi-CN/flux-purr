...
- The App boot path renders the brand splash before power and runtime initialization proceed. The splash copies a checked-in RGB565 template generated from the official dark Logo asset; the static `FLUX PURR` wordmark and `FLUX_PURR_FW_VERSION` use 4x7 pixel bitmaps, with the version at `y=35` and centered on the wordmark's `x=105` visual center. The splash renderer owns this 4x7 glyph set; existing runtime 3x5 text call sites remain outside its scope. The Logo at `x=8,y=8`, wordmark at `x=60,y=10`, and version form one vertically centered composition spanning rows `8..42`, with seven logical pixels of top and bottom margin. The resulting frame has a fixed four-color palette and is replaced by the first normal Dashboard refresh without an artificial dwell. Key Test retains the calibration scene.
