...
- `REQ-BUZZER-ARBITRATION-001` 至 `REQ-BUZZER-ARBITRATION-007`：`firmware/src/buzzer.rs` 中的 `BuzzerArbiter` 是唯一的 cue 选择边界；模块私有的 `BuzzerController` 仅负责已选 cue 的 tone/rest 步骤与 PWM 输出。
...
- 通过 host-side 仲裁单元测试、固件运行时测试和 PWM 载波回归测试验证；不新增 Control Plane API、持久化字段或设备配置。
