...
|-----:|-------|--------|------|------|-------|
| - | FUSB302B 双硬件 PD Sink | 部分完成（2/3） | `fusb302b-dual-pd-sink/SPEC.md` | 2026-08-27 | Public `fusb302` PHY integration, read-only `0x9x` identification, `5V..28V`-guarded PPS runtime dispatch within the live APDO (currently `5V..21V`) with fixed-PDO fallback, contract-capped PWM, status contract, and netlist baseline are present; PPS `20V/3A` and `20V/5A` source HIL remains gated |
...
| ram-bringup-firmware | active | `ram-bringup-firmware/IMPLEMENTATION.md` | `ram-bringup-firmware/SPEC.md` | - | Separate all-RAM firmware and CLI-only physical preview/test path |
...
| s3-gc9d01-display-bringup | active | `s3-gc9d01-display-bringup/IMPLEMENTATION.md` | `s3-gc9d01-display-bringup/SPEC.md` | - | GC9D01 async display driver and ram-bringup baseline |
