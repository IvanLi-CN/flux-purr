...
- `flux-purr thermal flagship-tune` 已接管 owner-facing `100w / pps5a` flagship orchestration：按 `60 / 140 / 220°C` 目标顺序执行 target-local scout、offline retune、`current + evidence-specific predicted point` batch compare、promotion-gated `60s` hold confirm，并写出 canonical preliminary review bundle。真实 HIL 继续以 per-target budget 作为默认停止边界；dry-run 默认单轮验证 artifact contract，避免 fixture 在 220°C 动态 5s gate 下高速空转。
...
- 历史 `scripts/thermal_tuning*` Python 工具链已经从正式 owner-facing / HIL 执行面移除。当前 supported orchestration/report path 固定为 Rust CLI：`flux-purr thermal flagship-tune` 与 `flux-purr thermal report rerender-legacy`。
