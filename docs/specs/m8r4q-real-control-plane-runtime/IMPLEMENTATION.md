...
- `flux-purr thermal tune` 已接管 owner-facing `100w / pps5a` 5A full-batch orchestration；`flagship-tune` 只保留为兼容别名。默认 tuning anchors 为 `60 / 100 / 140 / 180 / 220°C`，validation targets 为 `80 / 120 / 160 / 240°C`。每个 tuning target 执行 target-local scout、offline retune、`current + evidence-specific predicted point` batch compare、promotion-gated `60s` hold confirm；所有 tuning targets 收口后，runner 用最终 review candidate profile 对 validation targets 执行独立 `60s` hold-confirm validation。validation 通过只记录 `validation_passed`，不新增锚点、不修改 profile。真实 HIL 继续以 per-target budget 作为默认停止边界；dry-run 可用于 artifact contract 验证。
...
- thermal self-test / flagship HIL 现在会对 live 温度读数执行 host-side sanity gate。若样本在该时间间隔内出现 implausible temperature jump，尤其是 `heaterOutputPercent=0` 或实际温度与 `heaterFilteredTempC` 严重背离时，run 会被记为 `temperature_sample_glitch`：样本与 summary 仍保留审计证据，但 validation 不再把该次尝试计入 overshoot / hold p2p / stable-window 评分，flagship 只把它当作一次可有界重试的环境故障。
...
- thermal tune 默认 tuning anchors 为 `60 / 100 / 140 / 180 / 220°C`，validation targets 为 `80 / 120 / 160 / 240°C`；支持的完整 self-test ladder 为 `60 / 80 / 100 / 120 / 140 / 160 / 180 / 200 / 220 / 240 / 250°C`。`300°C` 不属于首版验收。默认 host sampling interval 为 `300ms`，持续采样下限为 `3Hz`；accepted comparison bundle 可以使用更高采样率。
...
- 当前 100W 剩余工作是用 `flux-purr thermal tune` 收口 full-batch preliminary review：tuning anchors `60 / 100 / 140 / 180 / 220°C`，validation targets `80 / 120 / 160 / 240°C`。历史 `0% / 25% / 50%` approach-only 曲线若要引用，只应作为显式诊断背景而不是当前默认判据。
