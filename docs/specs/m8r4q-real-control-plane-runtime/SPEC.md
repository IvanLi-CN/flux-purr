...
- `flux-purr thermal flagship-tune`：执行 owner-facing 的 `100w / pps5a` flagship thermal tuning / preliminary review orchestration。默认目标集为 `60 / 140 / 220°C`，每个目标从 cooldown wait 开始计入 `20min` per-target budget，循环执行 target-local scout、offline retune、`current + evidence-specific predicted point` batch compare，并且只有满足 warmup `100%`、动态 full-speed-to-stable gate 与确认裕量的 candidate 才进入 `60s` hold confirm。若 scout 本身已经证明当前 profile 满足上述 gate 与确认裕量，Rust orchestration 允许直接把当前 profile 提升到 `60s` hold confirm，而不是再消耗一轮冗余 batch retry。真实 HIL 默认不得设置独立调参轮次硬上限；`--max-tuning-rounds` 只可用于显式 debug/retest。`--dry-run` 为 host artifact/contract 验证路径，允许默认限制为单轮以避免 fixture 不满足高温 5s gate 时高速空转。Host 还必须对 live 温度读数执行合理性检查：若单次样本在对应时间间隔内出现 implausible temperature jump，尤其是在 `heaterOutputPercent=0` 或 `currentTempC` 与 `heaterFilteredTempC` 明显背离时，必须把该次尝试记为 `temperature_sample_glitch`；该次 run 的 `samples.ndjson` / summary 必须保留审计证据，但不得把它当作 overshoot / hold p2p / stable-window 调参证据。flagship orchestration 允许对同一子步骤做一次有界 retry，但不得仅凭这种原始温度跳变触发 source-side power recovery。命令输出 canonical preliminary review bundle：`index.html + run.bundle.json + samples.ndjson + thermal-profile.accepted.json`，顶层必须回显 `kind=thermal_self_test_preliminary_bundle`、`bundleDisposition=preliminary_review`、`acceptedProfileRole=review_candidate_snapshot`、`selectedMode=100w`、`resolvedBank=pps5a`、`detectedSourceClass=pps5a`、`tuningBudgetSeconds` 与 `flagshipTargetsC`。Python `scripts/thermal_tuning*` owner-facing 调优/报告入口已经废弃并从正式执行面移除；真实 HIL、正式调优、正式报告生成与 owner-facing 交付只允许走 Rust CLI。
