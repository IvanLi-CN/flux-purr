...
- A capability-refresh ticket MUST remain pending while its capability response has led to an unresolved `Accept`/`PS_RDY` transaction; only a settled protocol state may publish its terminal result.
