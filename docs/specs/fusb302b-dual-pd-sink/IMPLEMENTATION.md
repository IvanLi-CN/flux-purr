...
- FUSB302BMPX currently observes a `5V..21V` PPS APDO with fixed-PDO PWM fallback; calibration requires a qualifying PPS contract. The adapter contract requires all future programmable requests to retain the absolute `5V..28V` guard before applying the live APDO boundary. A pending FUSB contract starts the normal runtime with heater output interlocked, drains bounded completed PD frames each service turn, retries `Get_Source_Capabilities`, and sends at most one SOP Soft Reset after its first unanswered query. This restores the PD message exchange without initiating a Sink Hard Reset or withdrawing VBUS.
