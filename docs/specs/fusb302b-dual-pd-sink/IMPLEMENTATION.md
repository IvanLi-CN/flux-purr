...
- FUSB302BMPX currently observes a `5V..21V` PPS APDO with fixed-PDO PWM fallback; calibration requires a qualifying PPS contract. The adapter contract requires all future programmable requests to retain the absolute `5V..28V` guard before applying the live APDO boundary. A pending FUSB contract starts the normal runtime with heater output interlocked and drains bounded completed PD frames each service turn. The first startup discovery after an MCU reset may issue exactly one SOP Soft Reset after an unanswered first `Get_Source_Capabilities`, synchronizing a retained Source session without dropping CC or VBUS; all other absent-response recovery remains bounded `Get_Source_Capabilities` retry without a Sink PD reset.
- The FUSB302B runtime treats local `RETRY_FAIL`, pending-contract timeout, and partial FIFO timeout as CC-preserving recovery: it clears the active contract, flushes the FIFO, and returns to bounded `Get_Source_Capabilities` discovery. PHY reinitialization remains restricted to observed VBUS loss and received PD reset, so a transient message failure cannot withdraw Rd and cause the source to cycle the Device supply.
