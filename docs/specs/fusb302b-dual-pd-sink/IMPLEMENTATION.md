...
- FUSB302BMPX currently observes a `5V..21V` PPS APDO with fixed-PDO PWM fallback; calibration requires a qualifying PPS contract. The adapter applies the operational `5.5V` floor and absolute `28V` ceiling before the live APDO boundary. A pending FUSB contract starts the normal runtime with heater output interlocked and drains bounded completed PD frames each service turn. Every absent Source Capabilities response remains bounded `Get_Source_Capabilities` retry without a Sink PD reset.
- The FUSB302B runtime treats local `RETRY_FAIL`, pending-contract timeout, and partial FIFO timeout as CC-preserving recovery: it clears the active contract and cached capabilities, flushes only the receive FIFO, and returns to bounded `Get_Source_Capabilities` discovery. PHY reinitialization is restricted to a received PD reset or an idle CC measurement proving detach, so a transient message failure cannot withdraw Rd and cause the source to cycle the Device supply.
