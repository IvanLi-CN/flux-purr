...
- PD service records a monotonic heartbeat only after each bounded poll-and-publish turn. The hardware watchdog may feed only when this heartbeat and the Front Panel runtime heartbeat have both advanced; a stalled PD owner cannot leave an electrically stable but stale contract unsupervised indefinitely.
