...
- When a newer command supersedes a deferred PD operation, the service processes the queued command before retrying deferred work. Once superseded, the old operation is not retransmitted and its ticket receives one terminal outcome; an explicit Idle command follows the same replacement ordering.
