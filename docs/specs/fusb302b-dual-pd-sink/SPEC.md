...
- FUSB302B's `GPIO7` interrupt net is reserved for a later event-driven path and shares `GPIO8`/`GPIO9` with the M24C64 EEPROM. Sink initialization retains Rd pull-downs on both CC pins, and the current policy safely polls the PHY's clear-on-read interrupt snapshot. While the controller remains powered, only a reported `I_VBUSOK` transition accompanied by `STATUS0.VBUSOK=0` is treated as physical detach; static status reads are not enough to restart CC. EEPROM record reads/writes, raw maintenance reads/writes, snapshot reads/digest, and verification must release the shared bus after every bounded chunk and service the FUSB302B before the next chunk; no EEPROM write-cycle delay or success-path flash mirror may starve receive or contract recovery.
...
- Pass condition: absent source capabilities and local receive faults retain CC, interlock heat, and retry discovery without initiating a Sink reset; only the powered-controller `I_VBUSOK` transition evidence may enter detach recovery, which clears the contract and re-enters controlled CC discovery.
