...
- Pass condition: absent source capabilities and local receive faults retain CC, interlock heat, and retry discovery without initiating a Sink reset; any low-VBUS observation interlocks immediately, while only a transition followed by the bounded confirmation enters detach recovery, and persistent low VBUS re-enters controlled CC discovery only once until VBUS is restored.
