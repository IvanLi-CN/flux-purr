...
- A PPS adjustment is continuous only when the exact request resolver selects the same source APDO as the confirmed active contract and preserves its operating-current request. An overlapping APDO range selected with different precedence is a discrete contract change. Heater control keeps PWM while a continuous adjustment awaits confirmation; it clears physical PWM before submitting a discrete voltage increase or mode change and restores it only after asynchronous confirmation. The PD service reports protocol state but does not impose a generic heating pause for contract transitions.
- The application power facade accepts one atomic Fixed/PPS request containing exact voltage and operating current, explicit capability refresh, and idle. Request construction rejects zero or misaligned values (Fixed: 50mV voltage and 10mA current steps; PPS: 100mV voltage and 50mA current steps); the adapter also rejects malformed requests and never rounds them. It exposes confirmed PPS adjustment ranges and source-capability summaries, but PDO/APDO object positions remain private to the PD adapter. Accepted requests complete asynchronously through bounded tickets; consumers subscribe to semantic state changes rather than polling other modules.
- When a newer command supersedes a deferred PD operation, the service processes the queued command before retrying deferred work. Once superseded, the old operation is not retransmitted and its ticket receives one terminal outcome; an explicit Idle command follows the same replacement ordering. If an in-flight contract fails, already-admitted queued requests at the same or lower owner priority and queued capability refreshes settle as `Superseded`; higher-priority requests and explicit Idle remain eligible and are processed in order.
- A command that replaces an in-flight Contract, Idle, or capability refresh must cancel the old local operation, flush the receive FIFO, and establish a fresh `Source_Capabilities` exchange before sending a replacement RDO. Old `Accept`/`PS_RDY` responses observed before that new capability boundary are ignored and cannot complete the replacement ticket.
- An explicit capability-refresh ticket is terminal only after the capability message has been observed and any resulting contract transaction is settled; `WaitingForAccept` and `WaitingForPsRdy` are not refresh-terminal states. A refresh that preserves a valid active contract updates capabilities without re-requesting that same contract. A refresh timeout completes as `TimedOut` and cannot reuse cached capabilities as success.
