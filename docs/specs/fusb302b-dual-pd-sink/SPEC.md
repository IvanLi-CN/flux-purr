...
- While waiting for `Source_Capabilities`, the Sink may retry `Get_Source_Capabilities` after the bounded retry interval. After the first unanswered query it may send exactly one SOP PD Soft Reset to restart the message exchange while preserving VBUS, then continue bounded re-querying. It must not initiate a PD Hard Reset solely because that response is absent or late: a Sink-initiated hard reset can withdraw the VBUS that powers the Device and turn a recoverable contract delay into a reboot loop. A received reset, detach, reject, wait, or controller fault remains an explicit local recovery and heater-interlock event.
