...
- Implementation: the existing Web flow is implemented, but release-signature enforcement and EEPROM-Only Persistence are not yet implemented.
- Lifecycle: active
...
- devd bundle API currently uses content-addressed import, a five-minute approval token, fixed port/lease/ROM/package/task binding, Rust `espflash 4.5.0` security probing, MCU configuration preservation, and per-segment ROM MD5. MCU configuration preservation is superseded by EEPROM-Only Persistence and must be removed; release signatures must become mandatory. The remaining runtime, ROM, reset, and verification behavior stays subject to the current `SPEC.md`.
...
- Previous devd HIL exercised MCU configuration preservation. It does not verify the current signature, EEPROM-only persistence, or CLI boundary requirements and must not be treated as their acceptance evidence. Browser HIL remains independently gated.
...
- Release-signature enforcement, removal of MCU configuration preservation, and the CLI local-control boundary require non-hardware verification before any new HIL.
