...
- devd bundle API 使用内容寻址导入、五分钟单次 approval token、固定端口/lease/ROM/包/任务绑定、Rust `espflash 4.5.0` 安全探测、配置保全和逐段 ROM MD5。update 在 ROM 探测前从运行时刷新 identity、显式停热并重新读取 status；当前温度未低于或等于 40 C 时保持 fail closed，等待下一次完整 preflight，而不是复位进入 ROM。独立 ROM 探测在建立连接后始终复位回 runtime。每个 bundle 事务仅在起始进入 ROM 一次，后续擦除、段写入、MD5、配置保全均保持 `no-reset`，全部验证后才复位到 runtime 一次。写后在同一原生串口会话中依次验证 runtime identity 与 install status，并为冷启动保留 45 秒专用验证窗口；Browser 在 8 秒初始化窗口内对 USB Serial/JTAG 的请求只发送一次，只有观察到 `boot_stage=runtime_ready` 后才重发同一请求一次，绝不以定时循环排队重复 JSONL 命令。固件对已接收请求的 JSONL 回复使用端点确认的分包发送，避免较长 identity/status 帧在临时 busy 时静默丢失。验证 I/O 异常直接结算为 `write_complete_unverified`，不得重新打开 USB Serial/JTAG 会话造成额外复位。
