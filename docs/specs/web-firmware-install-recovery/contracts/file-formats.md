...
- Exactly five files are permitted:
  - `manifest.json`
  - `manifest.sig`
  - `images/bootloader.bin`
  - `images/partition-table.bin`
  - `images/factory-app.bin`
- `manifest.json` MUST validate against `firmware-bundle.schema.json`; unknown fields are rejected. `manifest.sig` MUST be a 64-byte Ed25519 detached signature over the exact UTF-8 bytes of `manifest.json`; `signingKeyId` selects the product public key from the verifier's compiled-in trust ring.
...
- The Vite development proxy returns the same file contract for already signed release bundles. It may cache bundled or server-validated release bytes in process memory, but it must not wrap, expose, or overlay a development ELF as a `.fluxpurr-fw` bundle. A GitHub refresh failure leaves already validated bundled releases usable.
