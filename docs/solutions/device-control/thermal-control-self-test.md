...
Host-side measurement sanity is part of the same boundary. A self-test sample that jumps by an implausible temperature delta for the observed interval, especially while heater output is already `0%` or while actual temperature diverges far from `heaterFilteredTempC`, must be recorded as `temperature_sample_glitch`. That attempt stays auditable in `samples.ndjson`, but it is not valid thermal tuning evidence and must not contribute overshoot / hold-p2p / stable-window scoring. The flagship runner may spend one bounded retry on the same sub-step after this classification, but a raw temperature jump alone must not trigger source-side power recovery.
