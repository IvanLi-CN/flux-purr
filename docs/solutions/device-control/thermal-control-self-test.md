...
Because those extra fields widened each point materially, EEPROM persistence must not blindly serialize all 10 slots every time. The current implementation only encodes the actual configured point count, which preserves compatibility with the existing record size while still allowing richer per-point data.
