...
真实 daemon transport 的仪表盘数值必须以 daemon/firmware status 为准。mock 场景可以用本地温度仿真和乐观交互反馈，但这些覆盖不能套到 `devd` 设备上，否则页面会在硬件已经安全停机时继续显示旧的加热百分比或漂移温度。
...
- 长 smoke 或浏览器自动化在 artifact verify、WiFi provisioning、runtime readback 之间必须 heartbeat lease；不要用一个窄 TTL lease 跨多个慢步骤后再执行 mutating endpoint。
...
- host-requested response frame 必须可靠写出大于 USB FIFO 的 JSONL 帧；启动期应至少响应 identity/network，并对 runtime status 或 mutating command 返回可重试 `startup_busy`，避免 host 把初始化窗口误判为永久失联。
...
- 设备校准这类会改变控制输入的设置应走独立 command family，而不是塞进普通 runtime config。draft/active 双状态、物理参考值到 ADC-domain 的转换、apply 安全门禁和持久化边界都应在 firmware/native bridge 侧共享，Web 只展示和发起受控操作。
...
ADC 校准属于控制面设置，不属于 dashboard display offset。RTD 和 VIN 都应保存 sample points，让 firmware、daemon mock、CLI export/import 和 Web preview 可以从同一 package 重新拟合 gain/offset。应用 active calibration 前必须证明 heater 不在工作；RTD raw electrical fault checks 应留在校准前，避免把开路/短路校准成正常温度。

发布也应该按产品而不是组件切分。单一 `vX.Y.Z` Release 可以同时携带 Web、firmware 和 host-tools；release manifest 用每个组件的 `contentSha256`、asset `sha256`、protocol versions 和 `changedSincePrevious` 表达是否需要升级。这样既能保持用户看到的是一个 Flux Purr 版本，也能避免 firmware 未变时仍提示用户刷机。
...
- 使用 feature flags（例如 `web_serial`，以及未来真正落地后的 `net_http`）在 firmware identity 中明确 transport 能力；不要为尚未实现的 transport 提前声明 capability。
- WiFi PSK 和其他 secret 必须在 logs、traces、errors、UI history 中 redaction。
- 所有 log、trace、event 和 serial line buffer 都必须有上限。
- 烧录路径要 source-aware：浏览器可以烧录 remote image artifact；daemon 只能烧录本地磁盘存在的 artifact。ESP32-S3 ELF 应走 `espflash flash --after hard-reset`；只有带 explicit flash address 的 raw app binary 才走 `espflash write-bin`。
