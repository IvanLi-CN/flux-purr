...
Flux Purr 的当前真实控制面先收敛在 Web -> native `devd` -> USB JSONL。固件 release artifact 默认包含 `web_serial`，但不声明 direct firmware HTTP / event stream capability；direct `net_http` 只有在固件 HTTP server 真正实现、验证并进入 identity capabilities 后，才应出现在 artifact catalog、Web target 或 API 文档的当前能力列表里。
