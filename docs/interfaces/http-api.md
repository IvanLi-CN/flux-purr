...
- `flux-purr thermal tune --engine firmware --power-class pps3a|pps5a --device <id> [--output-dir <directory>]` runs the firmware-owned nine-target PPS workflow. The product path does not accept `auto`, `65w`, `100w`, source controls or external VBUS telemetry. The fixed target set is `60 / 80 / 100 / 120 / 140 / 160 / 180 / 220 / 240°C`; the execution order is `60, 240, 140, 100, 80, 120, 180, 160, 220`. `--engine host-reference` remains available as the independent legacy/reference algorithm, including its existing source/profile arguments, and is never used as a Web fallback.
