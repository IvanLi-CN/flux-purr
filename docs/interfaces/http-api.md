...
When the firmware advertises the development-only `buzzer_debug` capability, `frontpanelRoute`, `frontpanelPresentedRoute`, and `frontpanelPresentationCount` are optional status diagnostics. They record the requested UI route, the route of the last completed SPI panel flush, and the number of completed panel flushes since boot. If PD discovery finishes without a ready contract, the startup sequence reinitializes the panel and completes two Dashboard flushes before runtime-ready. These fields are transfer receipts, not LCD pixel readback; production firmware omits them.
