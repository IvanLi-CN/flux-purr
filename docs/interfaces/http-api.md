...
Calibration live control requires an active adjustable PPS contract. FUSB302BMPX exposes calibration only while its selected live PPS contract (currently within `5V..21V`) meets the `>=20V @ >=3A` performance tier; its fixed-PDO fallback remains heat-only. This path remains subject to the authorized HIL interoperability gate.
...
- `POST /api/v1/devices/:id/buzzer-test`: native USB endpoint for controlled module-level audio testing. It requires an active native-serial lease and firmware identity capability `buzzer_test`; LAN targets and the product Web control surface do not expose it. The standard firmware build includes the formal test session; `buzzer-observe` is an optional build feature for GPIO48 carrier readback.
...
`op` is exactly one of `trigger`, `run`, `stop`, or `status`. `trigger` requires one production cue from `ui_input | heater_on | heater_off | active_cooling_on | active_cooling_off | heater_reject | active_cooling_reject | protection_alarm | attention_reminder`; only `trigger` accepts `repeat=true`. `run` requires `scenario=feedback_coalesce|feedback_replace|active_cooling_retrigger`; `active_cooling_retrigger` submits the production `active_cooling_on` cue at 0, 15, and 30 ms through the normal arbiter. `stop` and `status` accept neither cue nor scenario. The endpoint serializes the matching USB JSONL frame and returns `{ state, scenario?, cue?, repeat, activeCue?, trace[], outputTrace? }`. With `buzzer-observe`, each `outputTrace` entry records the logical requested frequency, the carrier derived from MCPWM timer2's readback prescaler and period, `observedFrequencyHz`, `observedRisingEdges`, `observedWindowMs`, duty, and cue generation after the normal GPIO48 output task writes it. The observed fields come from a feature-only PCNT input counting GPIO48 pad edges over that step; they are a digital pad measurement, not an acoustic measurement. The trace is bounded to sixteen entries and records silence as `requestedFrequencyHz=null` while retaining the last timer configuration.

The diagnostic is a test session layered on the ordinary firmware runtime: it uses the same `BuzzerArbiter`, production cue patterns, `ProtectionAlarmCadence`, priority-2 real-time buzzer timing task, and GPIO48 output application. A repeating ordinary feedback cue is resubmitted through the arbiter immediately after each production pattern completes. `protection_alarm` is the normal active thermal-runaway alarm; with `repeat=true` it starts immediately and repeats at the same one-second cadence. `attention_reminder` starts its first one-shot immediately and then uses the ordinary ten-second reminder cadence. `stop` is the explicit operator command that ends a repeated session. Firmware rejects test starts while heating, a real measurement fault, a thermal latch, or attention acknowledgement is pending. The endpoint never accepts frequency, duty cycle, raw cue steps, persistence controls, or a LAN request. The direct CLI waits until a finite cue or the first repeated cue is quiet before it requests `status` for an authoritative readback, so a queued trigger cannot be reported as the previous session state. Its interactive mode never polls during playback: repeated Enter or Space requests follow the same feedback-arbitration path as repeated hardware input, and `R` explicitly requests an idle readback after the operator is done listening.
...
- `flux-purr flash --port <serial-port> [--elf <local-elf>] [--skip-backup --confirm NO_EEPROM_BACKUP] [--keep-download-mode]`: 开发者直接串口烧录本地 ELF，默认先备份 EEPROM，不连接 devd；成对旁路无需 ROM 模式，跳过 ROM 探测和 EEPROM snapshot/归档后直接进入 espflash。`--keep-download-mode` 仅在设备已手动进入 ROM 下载模式时使用，烧录前后都不复位设备。
