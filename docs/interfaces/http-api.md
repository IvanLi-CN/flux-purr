...
The diagnostic is a test session layered on the ordinary firmware runtime: it uses the same `BuzzerArbiter`, production cue patterns, `ProtectionAlarmCadence`, priority-2 real-time buzzer timing task, and GPIO48 output application. A repeating ordinary feedback cue is resubmitted through the arbiter immediately after each production pattern completes. `protection_alarm` is the normal active thermal-runaway alarm; with `repeat=true` it starts immediately and repeats at the same one-second cadence. `attention_reminder` uses the ordinary ten-second reminder cadence. `stop` is the explicit operator command that ends a repeated session. Firmware rejects test starts while heating, a real measurement fault, a thermal latch, or attention acknowledgement is pending. The endpoint never accepts frequency, duty cycle, raw cue steps, persistence controls, or a LAN request. The direct CLI waits until a finite cue or scenario is quiet before it requests `status` for readback. Its interactive mode never polls during playback: repeated Enter or Space requests follow the same feedback-arbitration path as repeated hardware input, and `R` explicitly requests an idle readback after the operator is done listening.
...
- `flux-purr buzzer test --device <id> --cue ui-input|heater-on|heater-off|active-cooling-on|active-cooling-off|heater-reject|active-cooling-reject|protection-alarm|attention-reminder [--repeat]`
...
- `flux-purr buzzer play --device <id> [--devd <url>]` or `--hardware <saved-id> [--devd <url>]`: interactive native-terminal diagnostic session. The Justfile forwards the same explicit selector as `just buzzer-play --device <id> [--devd <url>]` or `just buzzer-play --hardware <saved-id> [--devd <url>]`. It displays the actual task-owned session state and arbitration trace; its production-cue catalogue covers `ui_input`, `heater_on`, `heater_off`, `active_cooling_on`, `active_cooling_off`, `heater_reject`, `active_cooling_reject`, `protection_alarm`, and `attention_reminder`. In a TTY, Up/Down/Home/End and left pointer clicks select an item; Enter or Space plays the selected cue once, and OS key-repeat submits the same request repeatedly so the production arbiter applies its ordinary feedback coalescing and replacement rules. `C` starts continuous playback for a cue, `S` stops, `R` refreshes, and `Q` or Escape exits. The session also runs the fixed feedback-coalescing and feedback-replacement scenarios. When a repeated session is running, its default Enter/Space action is stop; an ordinary one-shot feedback is never stopped or synthetically restarted by the CLI. Non-TTY input retains the line menu for scripts and automated checks.
