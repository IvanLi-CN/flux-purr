...
- `POST /api/v1/devices/:id/buzzer-test`: native USB endpoint for controlled module-level audio testing. It requires an active native-serial lease and a diagnostic firmware identity capability `buzzer_test`; LAN targets and the product Web control surface do not expose it. The default production firmware omits this endpoint; `buzzer-observe` is an optional build feature for GPIO48 carrier readback.
