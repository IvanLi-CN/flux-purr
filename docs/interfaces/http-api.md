...
When the firmware advertises the development-only `buzzer_debug` capability, `frontpanelRoute`, `frontpanelPresentedRoute`, and `frontpanelPresentationCount` are optional status diagnostics. They record the requested UI route, the route of the last completed SPI panel flush, and the number of completed panel flushes since boot. The App runtime renders Dashboard as the initial frame and presents it once after startup measurements, without a second panel initialization; the KeyTest runtime alone renders the calibration scene. These fields are transfer receipts, not LCD pixel readback; production firmware omits them.
