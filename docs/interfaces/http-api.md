...
- `POST /api/v1/devices/:id/buzzer-debug`: feature-gated native USB endpoint for controlled module-level audio testing. It requires an active native-serial lease and firmware identity capability `buzzer_debug`; ordinary firmware, LAN targets, and the product Web control surface do not expose it.
