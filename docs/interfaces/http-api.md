...
`op` is exactly one of `trigger`, `run`, `stop`, or `status`. `trigger` requires one production cue from `ui_input | heater_on | heater_off | active_cooling_on | active_cooling_off | heater_reject | active_cooling_reject | protection_alarm | attention_reminder`; only `trigger` accepts `repeat=true`. `run` requires `scenario=feedback_coalesce|feedback_replace`; `stop` and `status` accept neither cue nor scenario. The endpoint serializes the matching USB JSONL frame and returns `{ state, scenario?, cue?, repeat, activeCue?, trace[], outputTrace[] }`. Each `outputTrace` entry records the logical requested frequency, the carrier derived from MCPWM timer2's readback prescaler and period, duty, and cue generation after the normal GPIO48 output task writes it. The trace is bounded to sixteen entries and records silence as `requestedFrequencyHz=null` while retaining the last configured carrier.
