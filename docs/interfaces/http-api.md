...
- `flux-purr thermal tune --device <id> --source-id <bench-source-id> --source-url <lan-url> --profile-mode 100w --source-power-watts 100` runs the Rust-owned 5A full-batch preliminary review workflow. Defaults tune anchors `60 / 100 / 140 / 180 / 220°C`, then validate `80 / 120 / 160 / 240°C` with the final review candidate profile. Validation targets appear in the report and `samples.ndjson` but do not become tuning anchors when they pass.
