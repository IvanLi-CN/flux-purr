...
export function listKnownWebSerialDevices(
  ...
  try {
    ...
    return value
      .map(normalizeKnownWebSerialDevice)
      .filter((device): device is KnownWebSerialDevice => device !== null)
  } catch {
    return []
  }
}
...
export function rememberKnownWebSerialDevice(
  ...
  const normalized = normalizeKnownWebSerialDevice(device)
  if (!storage || !normalized) return
  const devices = listKnownWebSerialDevices(storage)
  const next = [
    normalized,
    ...devices.filter((candidate) => candidate.deviceId !== normalized.deviceId),
  ]
...
function normalizeKnownWebSerialDevice(value: unknown): KnownWebSerialDevice | null {
  if (!value || typeof value !== 'object') return null
  const device = value as Record<string, unknown>
  if (typeof device.deviceId !== 'string' || device.deviceId.trim().length === 0) return null
  const deviceId = device.deviceId.trim()
  return {
    deviceId,
    hostname:
      typeof device.hostname === 'string' && device.hostname.trim().length > 0
        ? device.hostname.trim()
        : deviceId,
    firmwareVersion:
      typeof device.firmwareVersion === 'string' && device.firmwareVersion.trim().length > 0
        ? device.firmwareVersion.trim()
        : 'unknown',
    buildId:
      typeof device.buildId === 'string' && device.buildId.trim().length > 0
        ? device.buildId.trim()
        : 'unknown',
  }
}
