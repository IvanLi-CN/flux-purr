...
// Indirection would add allocation to the bounded no_std request path.
#[allow(clippy::large_enum_variant)]
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum HttpReadGate {
    Respond(LightHttpResponse),
    Snapshot {
...
impl NetHttpState {
    ...
    pub fn gate_light_read(&mut self, request: HttpRequest<'_>) -> HttpReadGate {
        if request.method == HttpMethod::Options {
            return HttpReadGate::Respond(self.light_preflight(request));
        }
        ...
        match endpoint {
            LanEndpoint::Health => HttpReadGate::Respond(self.light_health_response(request)),
            LanEndpoint::Pairing => HttpReadGate::Respond(self.light_pairing_response(request)),
            LanEndpoint::Identity | LanEndpoint::Network => {
                if self.authorized(request.authorization) {
                    ...
                    HttpReadGate::Respond(self.light_error_response(
                        request.origin,
                        401,
...
mod tests {
    ...
    fn light_read_gate_keeps_snapshot_reads_out_of_the_control_mailbox() {
        ...
        let health = state.gate_light_read(req(HttpMethod::Get, "/health"));
        assert!(matches!(health, HttpReadGate::Respond(_)));

        let unauthorized = state.gate_light_read(req(HttpMethod::Get, "/api/v1/network"));
        assert!(matches!(
            unauthorized,
            HttpReadGate::Respond(LightHttpResponse { status: 401, .. })
        ));

        let network = state.gate_light_read(HttpRequest {
        ...
        assert!(matches!(
            state.gate_light_read(HttpRequest {
                authorization: Some(bearer),
