...
- Buzzer control:
  ...
  - each protection cue retains its established `2300Hz`, silence, `1850Hz`, silence pulse rhythm; the real-time GPIO48 owner changes the timer only for the two audible steps and preserves the carrier across each silence/replay boundary when the frequency is unchanged
  ...
  - a dedicated Embassy task in a priority-2 software-interrupt executor owns cue arbitration, cue-step deadlines and every GPIO48 duty update. Each cue starts from its first note; the task cannot skip a tone/rest step when delayed. GPIO48 retains its MCPWM phase across duty-zero silence gaps when the selected next tone uses the active carrier frequency; a different next audible frequency must reconfigure the timer so a previous frequency stage cannot continue
  - a firmware build with the non-default `buzzer-debug` feature advertises `buzzer_debug` over native USB JSONL for controlled module-level audio testing. It submits production cue IDs and fixed feedback-arbitration scenarios to the same real-time task and `BuzzerArbiter`; `protection_alarm --repeat` uses the normal one-second `ProtectionAlarmCadence`, while `attention_reminder` uses its normal ten-second cadence. Its bounded diagnostic result includes a read-only MCPWM timer2 output trace, so the logical cue output can be compared with the carrier actually configured for GPIO48. It is interlocked while heating, a real thermal fault, a fault latch, or thermal attention is active. The feature only adds this USB test session to the standard runtime and does not alter GPIO47/GPIO48 initialization, cue patterns, persistence, sensor handling, heater control, LAN endpoints, or product Web controls
- PD policy:
