...
- Buzzer control:
  ...
  - the same development build adds optional `frontpanelRoute`, `frontpanelPresentedRoute`, and `frontpanelPresentationCount` status fields. They report completed SPI panel transfers, not LCD pixel readback. If PD is unavailable at startup, the runtime keeps heating interlocked, reinitializes the panel, and completes two Dashboard transfers before reporting runtime ready
- PD policy:
