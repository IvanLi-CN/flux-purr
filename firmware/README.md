...
- Heater control:
  ...
  - formal thermal tuning allocates its bounded unacknowledged trace window, candidate workspace, and paged response snapshots explicitly from the board's 2 MiB PSRAM; allocation failure is reported instead of falling back to internal RAM, which retains only real-time control state, pointers, and compact metadata
