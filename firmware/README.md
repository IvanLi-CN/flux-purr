...
- Buzzer control:
  ...
  - a firmware build with the non-default `buzzer-debug` feature advertises `buzzer_debug` over native USB JSONL for developer diagnostics. It accepts only fixed feedback cues and `feedback_coalesce` / `feedback_replace` scenarios through the arbiter; it is interlocked while heating, thermal fault, fault latch, or thermal attention is active. After display initialization, pre-runtime recovery keeps GPIO47 low while preserving this feedback-only diagnostic, so it cannot control heater PWM, safety cues, persistence, or LAN endpoints
  - the same development build adds optional `frontpanelRoute`, `frontpanelPresentedRoute`, and `frontpanelPresentationCount` status fields. They report completed SPI panel transfers, not LCD pixel readback. The App runtime renders Dashboard as its initial frame and presents it once after startup measurements without reinitializing the panel; the KeyTest runtime alone renders the calibration scene
- PD policy:
