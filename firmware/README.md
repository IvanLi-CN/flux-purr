...
- Buzzer control:
  ...
  - a firmware build with the non-default `buzzer-debug` feature advertises `buzzer_debug` over native USB JSONL for controlled module-level audio testing. It submits production cue IDs and fixed feedback-arbitration scenarios to the same real-time task and `BuzzerArbiter`; repeating ordinary feedback re-enters the arbiter after each complete production pattern, `protection_alarm --repeat` uses the normal one-second `ProtectionAlarmCadence`, and `attention_reminder` uses its normal ten-second cadence. Its bounded diagnostic result includes MCPWM timer2 configuration and feature-only PCNT measurements of GPIO48 pad edges, so the logical cue output, configured carrier, and emitted digital carrier can be compared directly. It is interlocked while heating, a real thermal fault, a fault latch, or thermal attention is active. The feature only adds this USB test session and PCNT observation to the standard runtime; production builds do not initialize the probe and neither build exposes raw PWM control
- PD policy:
