...
#[cfg(target_os = "macos")]
fn is_usb_serial_jtag_path(port_name: Option<&str>) -> bool {
    matches!(port_name, Some(name) if name.starts_with("/dev/cu.usbmodem"))
}
...
impl io::Write for TTYPort {
    ...
    fn flush(&mut self) -> io::Result<()> {
        // macOS USB Serial/JTAG can leave tcdrain waiting indefinitely after
        // the driver has accepted a write. USB preserves write ordering, so
        // treating flush as a no-op avoids making ROM flashing uninterruptible.
        #[cfg(target_os = "macos")]
        if is_usb_serial_jtag_path(self.port_name.as_deref()) {
            return Ok(());
        }
...
#[cfg(target_os = "macos")]
#[test]
fn usb_serial_jtag_flush_path_is_narrow() {
    assert!(is_usb_serial_jtag_path(Some("/dev/cu.usbmodem2111401")));
    assert!(!is_usb_serial_jtag_path(Some("/dev/cu.usbserial-unrelated")));
    assert!(!is_usb_serial_jtag_path(None));
}
