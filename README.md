...
The branch protection contract is declared in [.github/quality-gates.json](.github/quality-gates.json). GitHub should protect `main`, require signed commits and the `Validate PR labels`, `Release completion`, `Firmware checks`, `DEVD checks`, `Web checks`, and `Worktree bootstrap` checks. The release controller uses the dedicated GitHub App configured through repository variable `RELEASE_APP_ID` and secret `RELEASE_APP_PRIVATE_KEY`; that App is the only ruleset bypass actor and has only `contents: write`. `GITHUB_TOKEN` remains read-only for release-intent lookup. No GitHub Environment is used.
