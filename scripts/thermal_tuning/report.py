...
<body>
  <main class="shell">
    ...
    <div class="section-head">
      <div>
        <h2>测试结果</h2>
        <p>只展示本次旗舰目标；full-speed-to-stable 按目标温度使用动态门槛：≤150°C 为 10s，>150°C 为 5s。</p>
      </div>
  ...
  function sampleTooltip(sample,items,title){{const rows=items.map(point=>`<div class="tip-row"><span><span class="tip-dot" style="color:${{point.color}}">●</span> ${{point.name}}</span><strong>${{tooltipValue(point,point.item,{{}})}}</strong></div>`).join('');return `<div class="tip-title">${{title}}</div><div>${{fmt(sample.x,2)}} s · 相位 ${{sample.phase?.toUpperCase()||'SAMPLE'}}</div><div>滤波 ${{fmt(sample.filtered,2)}}°C · 请求 ${{fmt(sample.requestV,2)}}V</div><div>电源 ${{fmt(sample.sourceVoltageV,2)}}V / ${{fmt(sample.sourceCurrentA,2)}}A / ${{fmt(sample.sourcePowerW,2)}}W</div>${{rows}}`;}}
  ...
  function renderRoundDetail(){{
    ...
    document.querySelector('#selectedRoundFacts').innerHTML=[
      ...
      ['warmup 实际 100%',round.result?.warmupOutputFull===true?'是':'否'],
